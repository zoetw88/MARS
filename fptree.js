...
class FPTree{
    ...
    fromTransactions(transactions) {
        ...
        transactions.forEach((transaction) => {
            let items = transaction
                ...
                .sort((a, b) => {
                let res = this.supports[JSON.stringify(b)] - this.supports[JSON.stringify(a)];
                if (res == 0)
    ...
    getConditionalFPTree(item) {
        let start = this._firstInserted[JSON.stringify(item)];
        if (!start)
            return null;
        let s = this.supports[JSON.stringify(item)];
        let conditionalTreeSupports = {};
        let prefixPaths = this._getPrefixPaths(start, s, (i, count) => {
            conditionalTreeSupports[JSON.stringify(i)] = (conditionalTreeSupports[JSON.stringify(i)] || 0) + count;
        });
        let ret = new FPTree(conditionalTreeSupports, this._support).fromPrefixPaths(prefixPaths);
    ...
    getPrefixPath(node, onPushingNewItem) {
        if (!this._isInit)
            throw new Error('Error building the FPTree');
        let path = this._getPrefixPath(node, node.support, onPushingNewItem);
        if (path.length === 0)
            return;
        return {
    ...
    _getPrefixPaths(node, count, onPushingNewItem, prefixPaths = []) {
        let prefixPath = this.getPrefixPath(node, onPushingNewItem);
        if (prefixPath)
    ...
    _updateLastInserted(key, child) {
        let last = this._lastInserted[key];
        if (last)
    ...
    _updateFirstInserted(key, child) {
        let first = this._firstInserted[key];
        if (!first)
