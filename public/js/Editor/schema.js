...
export const nodes = {
  ...
  ordered_list: {
    content: 'list_item+',
    group: 'block',
    attrs: { order: { default: 1 }, tight: { default: false }, ychange: { default: null }},
    parseDOM: [{
      tag: 'ol'
    }],
    toDOM(node) {   
      return ['ol',calcYchangeDomAttrs(node.attrs), 0];
    },
  },
  ...
  bullet_list: {
    content: 'list_item+',
    group: 'block',
    attrs: { tight: { default: false },ychange: { default: null } },
    parseDOM: [{ tag: 'ul' }],
...
export const marks = {
  ...
  link: {
   ...
   strikethrough: {
    parseDOM: [{ tag: 's' },{ style: 'text-decoration', }],
    toDOM() { return ['s',0]; },
  },
  underline: {
    parseDOM: [{ tag: 'u' }],
