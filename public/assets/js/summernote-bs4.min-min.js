...
'object'==typeof exports&&'undefined'!=typeof module?e(require('jquery')):'function'==typeof define&&define.amd?define(['jquery'], e):e((t=t||self).jQuery);
  ...
  'use strict'; function e(t) {
  ...
  }; const j=U('<div class="note-editor note-frame card"/>'); const K=U('<div class="note-toolbar card-header" role="toolbar"></div>'); const W=U('<div class="note-editing-area"/>'); const q=U('<textarea class="note-codable" role="textbox" aria-multiline="true"/>'); const V=U('<div class="note-editable card-block" contentEditable="true" role="textbox" aria-multiline="true"/>'); const G=U(['<output class="note-status-output" aria-live="polite"/>', '<div class="note-statusbar" role="status">', '  <output class="note-status-output" aria-live="polite"></output>', '  <div class="note-resizebar" role="seperator" aria-orientation="horizontal" aria-label="Resize">', '    <div class="note-icon-bar"/>', '    <div class="note-icon-bar"/>', '    <div class="note-icon-bar"/>', '  </div>', '</div>'].join('')); const _=U('<div class="note-editor"/>'); const Z=U(['<div class="note-editable" contentEditable="true" role="textbox" aria-multiline="true"/>', '<output class="note-status-output" aria-live="polite"/>'].join('')); const Y=U('<div class="note-btn-group btn-group">'); const Q=U('<div class="dropdown-menu" role="list">', function(t, e) {
    const o=Array.isArray(e.items)?e.items.map(function(t) {
      const o='string'==typeof t?t:t.value||''; const n=e.template?e.template(t):t; const i='object'==typeof t?t.option:void 0; return '<a class="dropdown-item" href="#" data-value="'+o+'"'+(void 0!==i?' data-option="'+i+'"':'')+' role="listitem" aria-label="'+o+'">'+n+'</a>';
    }).join(''):e.items; t.html(o).attr({'aria-label': e.title});
  }); const J=U('<div class="dropdown-menu note-check" role="list">', function(t, e) {
    const o=Array.isArray(e.items)?e.items.map(function(t) {
      const o='string'==typeof t?t:t.value||''; const n=e.template?e.template(t):t; return '<a class="dropdown-item" href="#" data-value="'+o+'" role="listitem" aria-label="'+t+'">'+nt(e.checkClassName)+' '+n+'</a>';
    }).join(''):e.items; t.html(o).attr({'aria-label': e.title});
  ...
  }); const ot=U('<div class="form-check"></div>', function(t, e) {
  ...
  }; var it={editor: j, toolbar: K, editingArea: W, codable: q, editable: V, statusbar: G, airEditor: _, airEditable: Z, buttonGroup: Y, dropdown: Q, dropdownButtonContents: function(t) {
    return t;
  }, dropdownCheck: J, palette: X, dialog: tt, popover: et, icon: nt, checkbox: ot, options: {}, button: function(e, o) {
    return U('<button type="button" class="note-btn btn btn-light btn-sm" role="button" tabindex="-1">', function(e, o) {
    ...
    let t; let e; let o; let n; if (ft.isW3CRangeSupport) {
      const i=document.getSelection(); if (!i||0===i.rangeCount) return null; if (Et.isBody(i.anchorNode)) return null; const r=i.getRangeAt(0); t=r.startContainer, e=r.startOffset, o=r.endContainer, n=r.endOffset;
    } else {
...
this.options.disableDragAndDrop?(this.documentEventHandlers.onDrop=function(t) {
  ...
  }(); ft.hasCodeMirror&&(Ot=window.CodeMirror); const qt=function() {
    function t(t) {
